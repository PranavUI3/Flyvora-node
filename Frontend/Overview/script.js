...
const indexTrendChart = new Chart(
  document.getElementById("chart-index-trend"),
  {
    type: "line",
    data: {
...
const leadTimeChart = new Chart(document.getElementById("chart-lead-time"), {
  ...
  options: {
    ...
    scales: {
      ...
      y: {
        ...gridOptions,
        ticks: { ...gridOptions.ticks, callback: (v) => "₹" + v },
      },
    },
  },
});
...
const airlineComparisonChart = new Chart(
  document.getElementById("chart-airline-comparison"),
  {
    type: "bar",
    data: {
...
const DEMO_ANOMALIES = [
  {
    route: "DEL-CCU",
    detail: "Fare spiked to ₹18,400 (4.1× median)",
    severity: "High",
  },
  {
    route: "BOM-BLR",
    detail: "Fare spiked to ₹9,200 (2.3× median)",
    severity: "High",
  },
  {
    route: "MAA-DEL",
    detail: "Gap in observations for T+7 window",
    severity: "Medium",
  },
  {
    route: "BLR-HYD",
    detail: "Duplicate quotes detected, auto-resolved",
    severity: "Low",
  },
];
...
const severityStyle = {
  ...
  Medium:
    "bg-amber-400/15 text-amber-300 border-amber-400/40 shadow-[0_0_8px_rgba(251,191,36,0.2)]",
...
DEMO_ANOMALIES.forEach(({ route, detail, severity }) => {
  ...
  li.className =
    "flex items-start justify-between gap-3 bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 rounded-lg px-3 py-2.5 transition-all duration-200 cursor-pointer hover:translate-x-1";
  li.innerHTML = `
        <div>
...
function showTab(tab) {
  ...
  if (isAirlines) {
    ...
    tabBtnAirlines.className =
      "px-3 py-1 rounded-md bg-gradient-to-r from-blue-600/50 to-blue-600/25 text-blue-300 cursor-pointer transition-all duration-200 shadow-sm font-medium";
    tabBtnAlerts.className =
      "px-3 py-1 rounded-md text-slate-400 cursor-pointer transition-all duration-200 hover:text-slate-200 font-medium";
  } else {
    ...
    tabBtnAlerts.className =
      "px-3 py-1 rounded-md bg-gradient-to-r from-blue-600/50 to-blue-600/25 text-blue-300 cursor-pointer transition-all duration-200 shadow-sm font-medium";
    tabBtnAirlines.className =
      "px-3 py-1 rounded-md text-slate-400 cursor-pointer transition-all duration-200 hover:text-slate-200 font-medium font-medium";
  }
}
...
async function loadOverview() {
  ...
  const [summary, indexTrend, routes, leadTime, airlines, anomalies] =
    await Promise.all([
      FlyvoraApi.get("/dashboard/overview/summary"),
      FlyvoraApi.get("/analytics/index-trend"),
      FlyvoraApi.get("/routes/ranking", { days }),
      FlyvoraApi.get("/analytics/lead-time"),
      FlyvoraApi.get("/airlines/comparison", { days }),
      FlyvoraApi.get("/analytics/anomalies"),
    ]);
  setMetric("metric-airfare-index", summary.airfareIndex.toFixed(1));
  setMetric(
    "metric-inflation-rate",
    `${summary.inflationRate >= 0 ? "+" : ""}${summary.inflationRate.toFixed(1)}%`,
  );
  setMetric("metric-anomalies-detected", summary.anomaliesDetected);
  setMetric("metric-tracked-routes", summary.trackedRoutes);
  indexTrendChart.data.labels = indexTrend.labels;
  indexTrendChart.data.datasets[0].data = indexTrend.national;
  indexTrendChart.data.datasets[1].data = indexTrend.south;
  indexTrendChart.update();
  leadTimeChart.data.labels = leadTime.labels;
  leadTimeChart.data.datasets[0].data = leadTime.prices;
  leadTimeChart.update();
  airlineComparisonChart.data.labels = airlines.map((item) => item.name);
  airlineComparisonChart.data.datasets[0].data = airlines.map(
    (item) => item.avgFare,
  );
  airlineComparisonChart.update();
  renderRouteRanking(routes);
  alertsList.innerHTML = "";
  anomalies.forEach(({ route, detail, severity }) => {
    const li = document.createElement("li");
    li.className =
      "flex items-start justify-between gap-3 bg-white/[0.03] border border-white/5 rounded-lg px-3 py-2.5";
